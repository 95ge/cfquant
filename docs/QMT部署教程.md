...
- ### 2、在 Web 初始化或绑定页填写你的 QMT 安装目录，系统会自动将项目中的 cfquant 目录复制到 QMT 的 bin.x64\cfquant；如果目录不存在或权限不足，页面会提示异常。历史手动复制位置如下图：
...
- ### 7、在我们的网页界面中进行账号绑定，QMT 目录可以填写安装目录或 bin.x64，保存后确认核心包自动复制提示无异常，如图：
...
- ### 2、极致模式入口脚本为自包含脚本，不依赖 QMT 侧导入 cfquant 包；如需保留传统目录，也可以参考下图位置。保存账号绑定时系统仍会尝试写入身份配置并提示异常：
![alt text](image-4.png)
...
- ### 3、在 Web 绑定页分别填写两个 QMT 安装目录或 bin.x64，系统会自动将 cfquant 目录复制到两个 QMT 的 bin.x64\cfquant；如果某个目录不存在或权限不足，页面会提示异常。历史手动复制位置如下图：
![alt text](image-4.png)
...
- ### 7、在我们的网页界面中进行账号绑定，保存后确认两个 QMT 的核心包自动复制提示无异常，如图：
