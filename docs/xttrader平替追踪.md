...
2026-09-10 信用适配补充：五类信用查询已统一对象转换、账号校验及缺失字段标记；资金明细返回 `XtCreditDetail`，未了结负债优先使用 `get_unclosed_compacts` 并返回 `StkCompacts`。只读 async 已采用后台查询。SMT 仅在扩展终端返回明确业务结果时经事件派发回调，不再把受理编号当作成功回报；原生 SMT 异步协议仍未适配。下文为早期包签名检查记录，当前逐项结论以 [官网接口适配清单](xtquant原版接口适配清单.md) 为准。
