...
```text
启动cfquant.bat        启动本地服务
停止cfquant.bat        停止本地服务
重启cfquant.bat        重启本地服务
```

通常日常运维只需要使用 `重启cfquant.bat`。重启只会重启 Web 和 PipeHub，并保留 LTtx，避免外部 `cfquant` Python 库的自动发现和通信入口中断。
...
新版 `启动cfquant.bat` 会在启动后等待 Web 端口真正可连接。启动失败时窗口会停留，并输出最近的启动日志和错误日志，不再直接闪退。

`启动cfquant.bat`、`重启cfquant.bat` 和 `停止cfquant.bat` 是项目保留的中文入口。重启会保留 LTtx，只等待旧 Web 端口释放后再启动；停止表示完整退出，会停止 Web、PipeHub 和 LTtx。停止失败时会保留窗口，方便查看失败原因。

需要把错误完整留在当前窗口时，可以在命令行中运行：

```bat
启动cfquant.bat --foreground
```
