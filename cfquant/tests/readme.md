...
**5、7 号保留原有下单默认行为，运行前务必核对配置。排查连接时先把 5 号的 `CONNECT_ONLY` 改为 `True`；4 号默认只读，18 号默认跳过下单。**
...
9 至 17、19、20、22、23 号以及 `test_*.py` 是自动化测试，不是连接账号的手工示例。pytest 运行方式保持不变，正式产品的 CLI 及其测试不受本次调整影响。
