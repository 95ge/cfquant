#coding:gbk
#! /usr/bin/python
# Embedded same-account market entry. Do not load parent entry files at runtime.

import os as _cfquant_market_os

_CFQUANT_MARKET = "SZ"
_CFQUANT_CONFIG_FILENAME = "cfquant_bridge_config_SZ.json"


def _cfquant_market_base_dir():
    path = globals().get("__file__") or ""
    if path and not str(path).startswith("<"):
        try:
            return _cfquant_market_os.path.dirname(_cfquant_market_os.path.abspath(path))
        except Exception:
            return ""
    try:
        return _cfquant_market_os.path.abspath(_cfquant_market_os.getcwd())
    except Exception:
        return ""


def _cfquant_market_config_path():
    base = _cfquant_market_base_dir()
    parent = _cfquant_market_os.path.dirname(base) if base else ""
    if base and _cfquant_market_os.path.basename(base).lower() == "python" and parent:
        return _cfquant_market_os.path.join(parent, "bin.x64", _CFQUANT_CONFIG_FILENAME)
    if base:
        return _cfquant_market_os.path.join(base, _CFQUANT_CONFIG_FILENAME)
    return _CFQUANT_CONFIG_FILENAME
...
# Embedded market entry: CFQUANT_LITE_SZ.py
#coding:gbk
#! /usr/bin/python
# CFQUANT_LITE_SZ.py
...
class TxTradeBridge(object):
    ...
    def _status(self):
        ...
        status = {
            ...
            "qmt_runtime_entry_script": "CFQUANT_LITE_SZ.py",
    ...
    def _runtime_info(self):
        ...
        try:
            market = str(globals().get("QMT_MARKET") or os.environ.get("CFQUANT_MARKET") or RUNTIME_CONFIG.get("market") or "").strip().upper()
        except Exception:
            market = ""
        if market not in ("SH", "SZ"):
            market = ""
        try:
            market_role = str(RUNTIME_CONFIG.get("market_role") or ("trade" if market else "")).strip()
        except Exception:
            market_role = "trade" if market else ""
        try:
            market_parent_bridge_id = str(RUNTIME_CONFIG.get("market_route_parent_bridge_id") or "").strip()
        except Exception:
            market_parent_bridge_id = ""
        return {
            ...
            "runtime_entry_version": LITE_ENTRY_VERSION,
            "qmt_runtime_entry_version": LITE_ENTRY_VERSION,
            "entry_script": "CFQUANT_LITE_SZ.py",
            ...
            "market_route_parent_bridge_id": market_parent_bridge_id,
            "qmt_runtime_market": market,
            "qmt_runtime_market_role": market_role,
            "qmt_runtime_market_parent_bridge_id": market_parent_bridge_id,
            "pid": os.getpid(),
...
def _entry_base_dir():
    ...
    try:
        ...
        if (
            os.path.isfile(os.path.join(cwd, "CFQUANT_LITE_SZ.py"))
    ...
    for path in sys.path:
        ...
        if path and os.path.isdir(path):
            ...
            if (
                os.path.isfile(os.path.join(base, "CFQUANT_LITE_SZ.py"))
...
def _apply_runtime_config():
    global BRIDGE_ID, PIPE_NAME, RUNTIME_CONFIG_PATH, RUNTIME_CONFIG, RUNTIME_CHANNELS, QMT_MARKET, DEFAULT_ACCOUNT_ID
    ...
    if data.get("market"):
        ...
        if market in ("SH", "SZ"):
            QMT_MARKET = market
            source = str(os.environ.get("CFQUANT_MARKET_SOURCE") or "").strip()
            if not os.environ.get("CFQUANT_MARKET") or source in ("cfquant_market_entry", "cfquant_entry"):
...
if QMT_MARKET:
    _print_log("cfquant lite market route market:%s bridge_id:%s" % (QMT_MARKET, BRIDGE_ID))
