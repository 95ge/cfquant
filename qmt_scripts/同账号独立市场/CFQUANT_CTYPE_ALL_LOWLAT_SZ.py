...
# Embedded market entry: CFQUANT_CTYPE_ALL_LOWLAT_SZ.py
#coding:gbk
#! /usr/bin/python
...
def _entry_base_dir():
    ...
    try:
        ...
        if (
            os.path.isfile(os.path.join(cwd, "CFQUANT_CTYPE_ALL_LOWLAT_SZ.py"))
    ...
    for path in sys.path:
        ...
        if path and os.path.isdir(path):
            ...
            if (
                os.path.isfile(os.path.join(base, "CFQUANT_CTYPE_ALL_LOWLAT_SZ.py"))
...
def _publish_callback(event_name, obj):
    try:
        ...
        # Forward stock-order callbacks to the separate trade bridge so a
        # synchronous order-id lookup can wake immediately.
