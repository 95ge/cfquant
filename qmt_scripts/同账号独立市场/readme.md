...
部分券商的大 QMT 会按策略运行标的所在市场过滤 `get_trade_detail_data` 的账号数据。此时即使脚本文件是 `_SZ.py`，如果运行时挂在 `SH000300` 这类沪市标的上，持仓查询仍可能只返回沪市持仓。

建议两个入口分别挂对应市场的常用标的运行：

- 上海入口：使用 `SH000300`、`600000.SH` 等沪市标的。
- 深圳入口：使用 `SZ399001`、`000001.SZ` 等深市标的。

如果 Web 端只看到沪市持仓，而 QMT 客户端持仓页确认有深市持仓，优先检查深圳 QMT 中 `_SZ.py` 是否运行在深市标的上下文。
